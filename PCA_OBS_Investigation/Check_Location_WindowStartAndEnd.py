...
if __name__ == '__main__':
    ...
    save_path_epochs = "/data/p_02569/Images/SingleSubject_PCA-OBS_Dataset1/AffectedEpochs/"
    ...
    for cond_name in cond_names:  # Conditions (median, tibial)
        ...
        for subject in subjects:  # All subjects
            ...
            count = 0
            epoch_indices = []
            for sample_win in samples_fitstart:
            ...
            df.at[f'{subject}', f'{cond_name}_fit_start'] = count
            df.at[f'{subject}', f'{cond_name}_no_windows'] = len(samples_fitstart)

            count = 0
            for sample_win in samples_fitend:
                for sample_stim in samples_stimulation:
                    if abs(sample_win - sample_stim) <= samples_before_after:
                        count += 1
                        epoch_indices.append(samples_stimulation.index(sample_stim))
            df.at[f'{subject}', f'{cond_name}_fit_end'] = count

            # Save epochs affected by both window start and window end
            ...
            # print(epoch_indices)
            rfile = open(save_path_epochs + f'{subject_id}_{cond_name}_affectedepochs.pkl', 'wb')
            pickle.dump(epoch_indices, rfile)
            rfile.close()

    # print(df)
    with pd.ExcelWriter(save_path_excel+"Window_Overlap.xlsx", engine='openpyxl') as writer:
        # use to_excel function and specify the sheet_name and index
        # to store the dataframe in specified sheet
        df.to_excel(writer, sheet_name="Window_Overlap", index=True)
