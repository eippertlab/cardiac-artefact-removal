...
if __name__ == '__main__':
    ...
    for cond_name in cond_names:  # Conditions (median, tibial)
        ...
        for subject in subjects:  # All subjects
            ...
            plt.figure()
            plt.plot(epochs.times, epo_data.T)

            epochs_detrend = detrend(epo_data, type='constant', axis=1)
            plt.figure()
            plt.plot(epochs.times, epochs_detrend.T)

            epochs_detrend = detrend(epo_data, type='constant', axis=0)
            plt.figure()
            plt.plot(epochs.times, epochs_detrend.T)
            plt.show()
            exit()