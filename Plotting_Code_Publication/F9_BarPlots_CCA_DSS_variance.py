    ...
    for file_paths in [paths, cca_paths, dss_paths]:
        ...
        for file_path in file_paths:
            ...
            with h5py.File(file_path + fname, "r") as infile:
                var_med = infile[keywords[0]][()].reshape(-1)
                var_tib = infile[keywords[1]][()].reshape(-1)

                if data_type_counter == 0:
                    df_med_uncleaned[name] = var_med
                    df_tib_uncleaned[name] = var_tib
                elif data_type_counter == 1:
                    df_med_cca[name] = var_med
                    df_tib_cca[name] = var_tib
                elif data_type_counter == 2:
                    df_med_dss[name] = var_med
                    df_tib_dss[name] = var_tib
