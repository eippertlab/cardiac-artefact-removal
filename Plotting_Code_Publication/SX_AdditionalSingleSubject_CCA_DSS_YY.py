...
if __name__ == '__main__':
    ...
    for data_type in ['DSS', 'CCA']:
        image_path = f"/data/p_02569/Images/SingleSubjectYY_{data_type}_Dataset1/"
        ...
        trials = [False]  # [True, False, True, False]
        time = [False]  # [False, True, True, False]
        ...
        for reduced_trials, shorter_timescale in zip(trials, time):
            for cond_name in cond_names:  # Conditions (median, tibial)
                ...
                for subject in subjects:  # All subjects
                    ...
                    evoked_prep_cca = epochs.average()
                    ...
                    if reduced_trials:
                        epochs = epochs[0::4]
                    epochs = epochs.pick_channels([channel])
                    evoked_ssp5_cca = epochs.average()
                    ...
                    # Uncleaned
                    if data_type == 'CCA':
                        ax1.plot(evoked_prep.times[:-1], evoked_prep_cca.get_data().T,
                                 color=pal[0])
                    elif data_type == 'DSS':
                        ax1.plot(evoked_prep.times, evoked_prep_cca.get_data().T,
                                 color=pal[0])
                    ax1.set_ylabel('Cleaned SEP Amplitude (AU)')
                    ax1.set_xlabel('Time (s)')
                    if cond_name == 'median':
                        ax1.set_title(f'Uncleaned + {data_type}')
                    ax10.plot(evoked_prep.times, evoked_prep.data[0, :]*10**6,
                              linewidth=0.5, linestyle='dashed', color='black')
                    ax10.set_yticklabels([])

                    # ICA
                    if data_type == 'CCA':
                        ax2.plot(evoked_prep.times[:-1], evoked_ica_cca.get_data().T,
                                 color=pal[2])
                    elif data_type == 'DSS':
                        ax2.plot(evoked_prep.times, evoked_ica_cca.get_data().T,
                                 color=pal[2])
                    ax2.set_xlabel('Time (s)')
                    if cond_name == 'median':
                        ax2.set_title(f'ICA + {data_type}')
                    ax2.set_yticklabels([])
                    ax20.plot(evoked_prep.times, evoked_prep.data[0, :]*10**6,
                              linewidth=0.5, linestyle='dashed', color='black')
                    ax20.set_yticklabels([])

                    # SSP6
                    if data_type == 'CCA':
                        ax3.plot(evoked_prep.times[:-1], evoked_ssp5_cca.get_data().T,
                                 color=pal[3])
                    elif data_type == 'DSS':
                        ax3.plot(evoked_prep.times, evoked_ssp5_cca.get_data().T,
                                 color=pal[3])
                    ax3.set_xlabel('Time (s)')
                    if cond_name == 'median':
                        ax3.set_title(f'SSP + {data_type}')
                    ax3.set_yticklabels([])
                    ax30.plot(evoked_prep.times, evoked_prep.data[0, :]*10**6,
                              linewidth=0.5, linestyle='dashed', color='black')
                    ...
                    if reduced_trials and shorter_timescale:
                        fname = f"{subject_id}_{data_type}_SEPTimeCourse_{cond_name}_reducedtrials_shorter"
                    elif reduced_trials and not shorter_timescale:
                        fname = f"{subject_id}_{data_type}_SEPTimeCourse_{cond_name}_reducedtrials"
                    elif shorter_timescale and not reduced_trials:
                        fname = f"{subject_id}_{data_type}_SEPTimeCourse_{cond_name}_shorter"
                    else:
                        fname = f"{subject_id}_{data_type}_SEPTimeCourse_{cond_name}"
